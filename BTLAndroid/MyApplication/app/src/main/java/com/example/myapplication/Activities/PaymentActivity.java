...
public class PaymentActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(maban !=0 ){
            ...
            for (int i=0;i<thanhToanDTSList.size();i++){
                int soluong = thanhToanDTSList.get(i).getSoLuong();
                int giatien = thanhToanDTSList.get(i).getGiaTien();
    ...
    public void onClick(View v) {
        ...
        switch (id){
            case R.id.btn_payment_ThanhToan:
                boolean ktraban = banAnDB.CapNhatTinhTrangBan(maban,"false");
                boolean ktradondat = donDatDB.UpdateTThaiDonTheoMaBan(maban,"true");
                boolean ktratongtien = donDatDB.UpdateTongTienDonDat(madondat,String.valueOf(tongtien));
    ...
    private void HienThiThanhToan(){
        madondat = (int) donDatDB.LayMaDonTheoMaBan(maban,"false");
        thanhToanDTSList = thanhToanDB.LayDSMonTheoMaDon(madondat);
