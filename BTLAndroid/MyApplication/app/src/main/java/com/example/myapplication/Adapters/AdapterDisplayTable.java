...
public class AdapterDisplayTable extends BaseAdapter implements View.OnClickListener{
    ...
    Context context;
    int layout;
    List<BanDTS> banDTSList;
    ViewHolder viewHolder;
    BanAnDB banAnDB;
    DonDatDB donDatDB;
    FragmentManager fragmentManager;

    public AdapterDisplayTable(Context context, int layout, List<BanDTS> banDTSList){
        this.context = context;
        this.layout = layout;
        this.banDTSList = banDTSList;
        banAnDB = new BanAnDB(context);
        donDatDB = new DonDatDB(context);
        fragmentManager = ((HomeActivity)context).getSupportFragmentManager();
    }

    @Override
    public int getCount() {
        return banDTSList.size();
    }

    @Override
    public Object getItem(int position) {
        return banDTSList.get(position);
    }

    @Override
    public long getItemId(int position) {
        return banDTSList.get(position).getMaBan();
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(banDTSList.get(position).isDuocChon()){
            HienThiButton();
        }else {
            AnButton();
        }

        BanDTS banAnDTO = banDTSList.get(position);

        String kttinhtrang = banAnDB.LayTinhTrangBanTheoMa(banAnDTO.getMaBan());
        //đổi hình theo tình trạng
        if(kttinhtrang.equals("true")){
            viewHolder.imgBanAn.setImageResource(R.drawable.ic_tinhtrang_table);
        }else {
            viewHolder.imgBanAn.setImageResource(R.drawable.ic_table_custom);
        }
    ...
    public void onClick(View v) {
        ...
        int maban = banDTSList.get(vitri1).getMaBan();
        String tenban = banDTSList.get(vitri1).getTenBan();
        ...
        switch (id){
            case R.id.img_customtable_BanAn:
                ...
                banDTSList.get(vitri).setDuocChon(true);
                HienThiButton();
                break;
            ...
            case R.id.img_customtable_GoiMon:
                ...
                String tinhtrang = banAnDB.LayTinhTrangBanTheoMa(maban);

                if(tinhtrang.equals("false")){
                    //Thêm bảng gọi món và update tình trạng bàn
                    DonDatDTS donDatDTS = new DonDatDTS();
                    donDatDTS.setMaBan(maban);
                    donDatDTS.setMaNV(manv);
                    donDatDTS.setNgayDat(ngaydat);
                    donDatDTS.setTinhTrang("false");
                    donDatDTS.setTongTien("0");

                    long ktra = donDatDB.ThemDonDat(donDatDTS);
                    banAnDB.CapNhatTinhTrangBan(maban,"true");
